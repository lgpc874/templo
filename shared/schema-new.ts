...
export const users = pgTable("users", {
  ...
  role: text("role").default("buscador").notNull(),
  role_color: text("role_color").default("#6b7280"),
  spiritual_name: text("spiritual_name"),
  magical_name: text("magical_name"),
  initiation_date: timestamp("initiation_date"),
  initiation_completed: boolean("initiation_completed").default(false),
  initiation_progress: integer("initiation_progress").default(0),
...
export const courses = pgTable("courses", {
  ...
  required_role: text("required_role").notNull().default("buscador"),
  is_published: boolean("is_published").default(true),
  reward_role_id: text("reward_role_id"),
  reward_badge_id: bigint("reward_badge_id", { mode: "number" }),
  price: decimal("price", { precision: 10, scale: 2 }).default("0"),
  is_paid: boolean("is_paid").default(false),
  sort_order: integer("sort_order").default(0),
  created_at: timestamp("created_at").defaultNow(),
...
export const modules = pgTable("modules", {
  ...
  requires_submission: boolean("requires_submission").default(false),
  ritual_mandatory: boolean("ritual_mandatory").default(false),
  created_at: timestamp("created_at").defaultNow(),
  updated_at: timestamp("updated_at").defaultNow(),
});

// Progresso do usuário nos cursos
export const user_course_progress = pgTable("user_course_progress", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  user_id: bigint("user_id", { mode: "number" }).references(() => users.id).notNull(),
  course_id: bigint("course_id", { mode: "number" }).references(() => courses.id).notNull(),
...
export const submissions = pgTable("submissions", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  user_id: bigint("user_id", { mode: "number" }).references(() => users.id).notNull(),
  module_id: bigint("module_id", { mode: "number" }).references(() => modules.id).notNull(),
  text_response: text("text_response").notNull(),
...
export const user_badges = pgTable("user_badges", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  user_id: bigint("user_id", { mode: "number" }).references(() => users.id).notNull(),
  badge_id: bigint("badge_id", { mode: "number" }).references(() => badges.id).notNull(),
...
export const insertUserCourseProgressSchema = createInsertSchema(user_course_progress).omit({
  id: true,
  started_at: true,
  completed_at: true,
});
...
export type UserCourseProgress = typeof user_course_progress.$inferSelect;
